# scripts/cafe/update_fundamental_stu_series.py
# =============================================================================
# FUNDAMENTAL S&D (STU Global) — Série + Decomposição (Top 6) via USDA FAS PSD API
# Objetivo: gerar/atualizar data/cafe/series/fundamental_stu.json (consumido pelo site)
#
...
# PSD commodityCode para "Coffee, Green".
# Observação: se por algum motivo este código estiver diferente no seu ambiente,
# basta definir a env var PSD_COFFEE_CODE no workflow e não precisa mexer no script.
COFFEE_COMMODITY_CODE = os.getenv("PSD_COFFEE_CODE", "0711100")

# Attribute IDs conforme vw_coffee (DDL enviado)
ATTR_PRODUCTION = 28
ATTR_EXPORTS = 88
ATTR_DOM_CONS = 125
...
OUT_PATH = os.path.join("data", "cafe", "series", "fundamental_stu.json")
...
def now_utc_str() -> str:
    # Manter padrão já usado nos JSONs do projeto (YYYY-MM-DD HH:MM:SS)
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            resp = requests.get(
                url,
                headers=HEADERS,
                params={"api_key": API_KEY},  # compatibilidade com seu padrão antigo
                timeout=TIMEOUT_S,
            )
...
def pick_values(records: List[Dict[str, Any]], attr_id: int) -> Optional[float]:
    """
    Dado o retorno da API (lista de registros), encontra o 'value' do attributeId desejado.
    Se houver múltiplos, pega o último.
    """
...
def resolve_country_name(rec: Dict[str, Any]) -> str:
    # PSD API costuma trazer countryName; se não trouxer, cai para countryCode
...
def series_update(existing_points: List[Dict[str, Any]], refresh_map: Dict[int, Optional[float]]) -> List[Dict[str, Any]]:
    """
    Remove pontos dos anos em refresh_map e reinsere os recalculados.
    """
...
def build_decomp(
    ...
    """
    prod_by_year_country[year][countryCode] = (countryName, production_value)
    prod_world_by_year[year] = production_value
    """
    # média world 5y
    ...
    # construir para cada país (exclui World se aparecer como countryCode '00' em algum dataset)
    countries = set()
    ...
    # ordenar e top N (por w_prod_5y desc)
...
def main() -> None:
    ...
    # 1) marketYears disponíveis
    years = get_available_market_years(COFFEE_COMMODITY_CODE)
    ...
    # 6) Meta (manter padrão do arquivo existente)
    meta = {
        "id": "fundamental_stu",
    ...
    # preserva quaisquer chaves extras que você já tinha (se existirem) — sem inventar novas
    ...
    # logs úteis
    print("OK: fundamental_stu.json atualizado.")
    print(f"Commodity: {COFFEE_COMMODITY_CODE} | latest_year={latest_year} | refresh_years={refresh_years}")
